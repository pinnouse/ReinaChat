...
class Bot:
    ...
    def sample(self, a, randomness=1):
    ...
    def decode_sequence(self, input_seq):
        ...
        target_seq[0, 0, self.target_token_index["<GO>"]] = 1.
        ...
        while not stop_condition:
            ...
            sampled_token_index = self.sample(output_tokens[0, -1, :], 2)
            sampled_w = self.reverse_target_w_index[sampled_token_index]
            # print("sampled token index:", sampled_token_index, "word:", sampled_w)
            decoded_sentence += sampled_w + " "

            if sampled_w == "<EOS>" or len(decoded_sentence.split(' ')) > self.max_decoder_seq_length:
                stop_condition = True
    ...
    def sentence_to_seq(self, sentence):
        ...
        read_sentence = ""
        for i in range(min(self.max_encoder_seq_length, len(sentence))):
            w = sentence[i].lower()
            if w not in list(self.reverse_input_w_index.values()):
                w = "<UNK>"
